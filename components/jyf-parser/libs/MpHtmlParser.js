...
class MpHtmlParser {
  ...
  remove(node) {
    ...
    while (1) {
      ...
      if (this.section().toLowerCase() == name) {
        ...
        else if (name == 'title') this.title = this.data.substring(j + 1, this.i - 7);
  ...
  matchAttr(node) {
    ...
    for (var i = 0, len = styles.length; i < len; i++) {
      ...
      if (value.includes('-webkit') || value.includes('-moz') || value.includes('-ms') || value.includes('-o') || value.includes('safe')) style += `;${key}:${value}`;
    ...
    for (var key in styleObj) {
      ...
      // 转换 rpx
      else if (value.includes('rpx')) value = value.replace(/[0-9.]+\s*rpx/g, ($) => (parseFloat($) * screenWidth) / 750 + 'px');
  ...
  popNode(node) {
    ...
    if (node.c && (node.name == 'ul' || node.name == 'ol')) {
      if ((node.attrs.style || '').includes('list-style:none')) {
        for (let i = 0, child; (child = node.children[i++]); ) if (child.name == 'li') child.name = 'div';
      } else if (node.name == 'ul') {
        var floor = 1;
        for (let i = this.STACK.length; i--; ) if (this.STACK[i].name == 'ul') floor++;
        if (floor != 1) for (let i = childs; i--; ) node.children[i].floor = floor;
      } else {
