<template>
  <view class="w-100 h-100">
    ...
    <view style="padding: 30rpx 40rpx; padding-bottom: 100rpx">
      ...
      <view class="text-color-assist font-size-sm" style="margin-bottom: 20rpx"> 共{{ couponNum }}张优惠券 </view>
      <!-- 优惠券列表 begin -->
      <view class="d-flex flex-column w-100">
        <view class="coupon d-flex flex-column bg-white" v-for="(coupon, index) in package.coupons" :key="index" @tap="openCouponDetailModal(coupon)">
          ...
          <view class="bottom d-flex font-size-sm justify-content-between align-items-center">
            <view class="text-color-assist"> 使用时段：{{ coupon.detail && coupon.detail.coupon_use_time[0].use_time_start }}-{{ coupon.detail && coupon.detail.coupon_use_time[0].use_time_end }} </view>
      ...
      <view class="font-size-base text-color-base"> 售卖时间：{{ package.start_at.split(' ')[0] }}~{{ package.end_at.split(' ')[0] }} </view>
      <view class="font-size-base text-color-base" style="margin-bottom: 30rpx"> 购买限制：无限制 </view>
    ...
    <view class="pay-box d-flex just-content-center align-items-center position-fixed fixed-bottom bg-white">
      <button type="primary" class="pay-btn font-size-base text-color-white rounded-pill" style="width: 90%; height: 80rpx; line-height: 80rpx"> ￥{{ package.amount }}购买 </button>
    </view>
